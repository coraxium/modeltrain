...
BlockLightBarrier::BlockLightBarrier(unsigned int blockPin, unsigned int firstInputPin, unsigned int secondInputPin) {
...
void BlockLightBarrier::readSensors()
{
  ...
  if(DEBUG) {
	  ...
	  Serial.print(this->outer1State);
	  Serial.print(" ");
	  Serial.println(this->outer2State);
  }
  */
}
...
void BlockLightBarrier::doEnterBlock(boolean outer, boolean inner) {
  ...
  if(this->blockExit || this->blockState) {
	  return;
  }
  ...
  if (this->singleMode && outer && this->useAxisCounter)
  {
    ...
    if(DEBUG && true) {
    	Serial.print("ENTER: axisCounterDiff: ");
    	Serial.println(this->axisCounterDiff);
    }
  ...
  {
    this->blockState = true;
	Serial.println("ENTER: blockstate TRUE");
    return;
  }
...
void BlockLightBarrier::doExitBlock(boolean outer, boolean inner)
{
  ...
  if (this->singleMode && outer && this->useAxisCounter)
  {
    ...
    if(DEBUG && true) {
    	Serial.print("EXIT: axisCounterDiff: ");
    	Serial.println(this->axisCounterDiff);
    } 
    return;
  }
  ...
  /*
    if(DEBUG && true) {
    	Serial.print("EXIT: blockstate: ");
    	Serial.println(this->blockState);
    }  
	*/
  ...
  if (this->singleMode) {
    if (outer && this->blockState && !this->blockExit) {
      this->blockExit = true;
	  Serial.println("Blockexit TRUE");
    }
    else if (outer && this->blockState && this->blockExit) {

		// Monoflop reset
		this->monoflop = millis();
		//Serial.println("EXIT Blockexit Reset Monoflop");
    }/*
    else if (!outer && (this->monoflop + FLOPTIME > millis()) && this->blockState && this->blockExit) {
      this->blockExit = false;
      this->blockState = false;
	  Serial.println("EXIT blockexit END");
    }*/
    else if ((this->monoflop + FLOPTIME < millis()) && this->blockState && this->blockExit) {
      this->blockExit = false;
      this->blockState = false;
	  Serial.println("EXIT blockexit END");
    }
